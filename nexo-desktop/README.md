...
1. Subir versión en `package.json` (ej. `1.1.12-fix`).
2. Commit y push a rama principal.
3. Crear y subir tag de release:
   ```bash
   git tag v1.1.12-fix
   git push origin v1.1.12-fix
   ```
...
> Nota de hash: GitHub muestra `sha256` en la UI del asset, pero `latest.yml` usa `sha512` (base64). No tienen por qué verse iguales; el workflow ahora valida esa equivalencia automáticamente y publica `checksums.txt`.
...
- Trigger: push de tags `v*.*.* (incluye sufijos como -fix)` (ej. `v1.1.12-fix`).
...
## Subir cambios sin PR (git apply)

Si querés subir directo sin abrir PR:

1. En la copia donde trabajás, generar patch:
   ```bash
   ./scripts/release_no_pr.sh export HEAD~1
   ```
   Te crea `/tmp/nexo-1.1.12-fix.patch`.

2. En la copia/repo destino, aplicar patch:
   ```bash
   git apply --index /tmp/nexo-1.1.12-fix.patch
   git commit -m "Apply Nexo 1.1.12-fix updates"
   git push origin <rama-o-main>
   ```

3. Publicar release/tag:
   ```bash
   git tag v1.1.12-fix
   git push origin v1.1.12-fix
   ```

> Consejo: antes de publicar, revisar `latest.yml`, `.exe`, `.blockmap` y `checksums.txt` en el Release.
